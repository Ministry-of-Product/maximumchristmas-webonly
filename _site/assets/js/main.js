...
function createProductCard(product, isFake = false) {
  ...
  // Only add "Buy on Amazon" link for real products (if affiliateUrl exists)
  if (!isFake && product.affiliateUrl) {
...
async function loadLeaderboard(type) {
  ...
  try {
    ...
    data.leaderboard.forEach(product => {
      const card = createProductCard(product, isFake);
